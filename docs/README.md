...
Spring Boot application deployed on Kubernetes (Kind cluster) with Podman runtime.
...
```bash
make build              # Build the application with Gradle
make image              # Create Podman image (localhost/spink:1.0.1)
make image-load         # Load image into Kind cluster
make deploy             # Deploy to Kubernetes (2 replicas)
make wait               # Wait for pods to be ready
make curl               # Test with curl to localhost:30080
make show-ips           # Display all cluster IPs and routing info
```

### Cleanup

```bash
make clean-k8s          # Delete all Kubernetes resources (keep cluster)
make reset-cluster      # Delete cluster and recreate it from scratch
```

### Run Everything at Once

```bash
make all                # Complete pipeline: reset → build → deploy → test
```

**What `make all` does (in order):**
1. `make reset-cluster` — Deletes old cluster and creates fresh one
2. `make image` — Builds application and creates Podman image
3. `make image-load` — Loads image into the Kind cluster
4. `make clean-k8s` — Cleans any previous K8s resources
5. `make deploy` — Applies deployment and service manifests
6. `make wait` — Waits for pods to be healthy (90s timeout)
7. `make curl` — Tests the application with a health check request

### Advanced Networking Troubleshooting

```bash
make install-net-tools  # Install tcpdump, iproute2, ping, net-tools on all nodes
make test-net-1         # Real-time tcpdump on worker node (shows packets)
make test-net-2         # Detailed packet capture with analysis
```
